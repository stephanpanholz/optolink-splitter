...
Changes in contrast to the main repository:
 - poll_items in settings_ini.py is a hashmap now, so it is useable for the new write functionality, where poll_items can be used instead of the address
 - Modifications in optolinkvs2_switch.py regarding read polling
 - Major changes in  request_util.py regarding read and write commands (see command syntax below). 
 - Modifications in mqtt_util.py
...
NEW SYNTAX:
  - read hotwater temperature setpoint via cmnd if polling disabled:
    write topic vito/cmnd/read/temperatur_warmwasser_soll
	read value of topic vito/temperatur_warmwasser_soll
	
  - read hotwater temperature setpoint with polling enabled:
    read value of topic vito/temperatur_warmwasser_soll

  - write 48°C to the hotwater temperature setpoint:
	put value of 48 to the topic vito/cmnd/write/temperatur_warmwasser_soll
